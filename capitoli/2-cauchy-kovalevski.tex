...
\textbf{Il secondo elemento che giocherà un ruolo importante all'interno è il teorema di Ascoli-Arzelà, che svolge il ruolo di teorema di compattezza per sostituire il teorema di Heine-Borel per spazi matrici compatti (uno spazio metrico è compatto se e solo se è sequenzialmente compatto). Utile a dimostrare la chiusura di un insieme (anticipane bene l'utilità nella dimostrazione.}
\begin{definition}
...
\item
costruire degli insiemi $E_{j,n} \subseteq l^\infty$ chiusi e senza parte interna sfruttando $F_{a}$ e teorema di Ascoli-Arzelà(lemma \ref{lemma-e});
\item
...
\begin{proof}
sfruttando l'invarianza dell'operatore $L$ rispetto a $T(x,y,t)=(x-x_0,y-y_0,t-t_0-2xy_0+2x_0y)$, ovvero la validità della relazione: $L(u \,\circ\, T)=(Lu) \,\circ\, T$. Risolvere $Lu=f(t+2xy_0-2x_0y)$ è equivalente a risolvere $L(u \,\circ\, T)=f(t-t_0)$.
\end{proof}
...
g_j(x,y,t)=f'(t+2xy_j-2x_jy)
}
{F_a=\sum_{j=1}^{\infty} a_jc_jg_j \text{ converge uniformemente in } \mathbb{R}^3}
{\text{lo stesso vale per le derivate formali } D^{\alpha}F_a=\sum_{i=1}^{\infty} a_jc_jD^{\alpha}g_j}
\end{lemma}
...
&\leq ||a|| \, 2^{-j} \, M_{| \alpha |+1} \left(\frac{| \alpha |}{e}\right)^{| \alpha |}& \text{ poichè } \max\limits_{x \geq 0}\frac{x^{| \alpha |}}{e^x} = \left(\frac{| \alpha |}{e}\right)^{| \alpha |} \label{eq:5}\numberthis
\end{align*}
$D^\alpha F_a$ converge totalmente, e quindi anche uniformemente, essendo che la serie $$\sum_{j=1}^{\infty} \sup\limits_{\mathbb{R}^3} |a_jc_j D^{\alpha} g_j|$$ ha un termine generale minore o uguale della parte destra della disuguaglianza \eqref{eq:5}, la cui corrispondente serie numerica è ovviamente convergente.
...
\quad - \quad Lu=F_a \text{ in } A_{j,n}\\
...
Supponendo la tesi falsa, è possibile affermare che $\forall a \in l^\infty \; \exists \, A \in \mathbb{R}^3, \, u^* \in C^1(A,\mathbb{R})$ tali che in $A$ si ha che $Lu^*=F_a$ e $u^*$ ha le derivate prime continue secondo Hölder in $A$.
