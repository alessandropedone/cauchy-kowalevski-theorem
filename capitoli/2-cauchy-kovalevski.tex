...
\{E_{j,n}\}_{j,n \in \mathbb{N}_0} \subseteq l^{\infty} \text{ tali che } \forall a \in E_{j,n} \;\; \exists \, u \in C^1(A_{j,n}) \text{ tale che }\\
...
Supponendo la tesi falsa, è possibile affermare che $\forall a \in l^\infty \; \exists \, A \in \mathbb{R}^3, \, u^* \in C^1(A, R)$ tali che in $A$ si ha che $Lu^*=F_a$ e $u^*$ ha le derivate prime continue secondo Hölder in $A$.
...
In ultimo è chiaro che, essendo le derivate prime di $u$ continue secondo Hölder, esiste un $m$ abbastanza grande per cui valgono le condizioni rimanenti in \eqref{eq:9} con $m$ al posto di $n$ e prendendo $n=\max\{k,m\}$ l'implicazione è dimostrata.
