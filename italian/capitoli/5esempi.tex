...
Dopo aver visto il teorema di Cauchy-Kowalevski in tutte le sue forme più note, si concentra ora lo sguardo su tre esempi importanti  che aiutano a inquadrare meglio il ruolo che giocano le ipotesi e i limiti di questo teorema.
...
Dopo aver inquadrato questi strumenti, è arrivato il momento di entrare nel merito della discussione e lo si fa con uno schema che presenta per punti e a grandi linee il ragionamento da affrontare:
\begin{enumerate}
\item
...
Per osservare tale comportamento si considera il seguente problema di Cauchy per l'equazione di Laplace in due dimensioni al variare di $n$:
\begin{align*}
u_{xx}+u_{yy}&=0\\
...
si verifica un blow-up della soluzione $u_n$ del problema \eqref{eq:6}.
\begin{enumerate}[1.]
\item
...
Infatti, considerando il problema per ${n=\infty}$, ovvero con dati $u(x,0)=u_y(x,0)=0$, si nota immediatamente che l'unica soluzione analitica è $u\equiv0$, la quale è profondamente diversa dal comportamento asintotico di $u_n$.
