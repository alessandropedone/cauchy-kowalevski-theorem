...
\mathcal{O}\subseteq A \text{ aperto connesso}\\
...
\widetilde{y_h}=0 \quad \text{ su } \Gamma
\end{cases}
\end{equation*}
\end{frame}
...
$$\widetilde{y}_h(x,t)=u(x_1+\cdots +x_n,\,t)$$
con
...
\begin{frame}
Le matrici $A_j$ e $B$ saranno quindi ricavabili dalle espressioni\footnote{$i(\alpha)=\min_i{i:\alpha\neq 0}$}:
\begin{align*}
...
\begin{theorem}
\hpth{
\overline{\mathcal{O}}_0 \subseteq \mathcal{O}_1 \subseteq \mathbb{C}^n \text{ aperti connessi}\\
A_j, f, u_0 \text{ olomorfi in } z\\
A_j, f, u_0 \text{ continui in } t\\
\text{Pb:}
\begin{cases}
...
\exists \, \delta \in (0,T) : \exists !\, y \in C^1 \text{ sol. per } |t|<T
}
\end{theorem}
\end{frame}
...
\item Treves e Nirenberg per la ricerca di condizioni necessarie e/o sufficienti per l'esistenza di soluzioni locali
\item Hörmander la teoria degli operatori differenziali lineari (con particolare attenzione alla condizioni necessarie)
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Teorema di Holmgren}
Risultato di \textbf{unicità} delle soluzioni, anche non analitiche, per EDP lineari.
\begin{table}
...
\begin{cases}
D_t y = \sum A_j (x,t) D_{x_j}y+A_0(x,t)y +f(x,t) \\
y=0 \text{ per } t=0
\end{cases} \\
}{
