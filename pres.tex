...
% teoremi, definizoni, dimostrazioni
...
\author{Condidato: Alessandro Pedone\\ Relatore: Prof. Maurizio Grasselli}
...
\begin{frame}
\frametitle{Condizioni di Cauchy}
\begin{itemize}
\item Spesso utilizzate quando la superficie dei dati \textbf{non} è un bordo.
\item Necessitano anche le \textbf{derivate normali} ($D^k_\nu u$) della soluzione sulla superficie per determinarla univocamente.
...
\begin{itemize}
\item lineare: $L=\sum_{|\alpha |\leq k} a_\alpha \, D^\alpha$
\item quasi-lineare:
\item non lineare:
\item in forma normale:
\end{itemize}
...
Dato un operatore differenziale lineare $L=\sum_{|\alpha |\leq k} a_\alpha \, D^\alpha$
\begin{definition}
Forma caratteristica di $L$:\\ $\chi_L(x,\xi)=\sum\limits_{|\alpha |= k} a_\alpha(x) \, \xi^\alpha \quad \text{con} \quad x,\xi \in \mathbb{R}^n$
\end{definition}

\begin{definition}
Superficie caratteristica di $L$ in $x$:\\ $\text{char}_x (L)= \{ \xi \neq 0 : \chi_L(x,\xi)=0 \}$
\end{definition}
\end{frame}
...
\begin{frame}
\frametitle{Legame metodo delle caratteristiche}
Caso lineare
\end{frame}
